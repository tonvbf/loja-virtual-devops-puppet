...
class jenkins::cli {
  ...
  $initialAdminLogon = "${jenkins::localstatedir}/secrets/initialAdminLogon"
  $initialAdminPasswd = "${jenkins::localstatedir}/secrets/initialAdminPassword"

  exec { 'auth-admin':
    command => "bash -c \"echo \'admin:$(cat ${initialAdminPasswd})\' > ${initialAdminLogon}\"",
    path    => ['/bin', '/usr/bin'],
    creates => $initialAdminLogon,
    require => Service['jenkins'],
  }
  
  file { $initialAdminLogon:
    ensure  => file,
    require => Exec['auth-admin'],
  }
  ...
  if $::jenkins::cli_ssh_keyfile {
    ...
    $auth_arg = "-auth @${initialAdminLogon}"
#   $auth_arq = undef    
  }
